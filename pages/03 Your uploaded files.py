...
if st.session_state.is_authenticated == True:
    ...
    df_metadata_personal = st.dataframe(df_metadata[df_metadata['ORCID'] == st.session_state.orcid_user_info['id'].split('/')[-1]])
    ...
    df_with_checkboxes = pd.concat([pd.Series([st.checkbox("Checkbox 1", key='checkbox1')] * len(df_metadata_personal), name="update"),
                                    pd.Series([st.checkbox("Checkbox 2", key='checkbox2')] * len(df_metadata_personal), name="delete"),
